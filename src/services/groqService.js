 ...
 * groqService.js
 *
 * EduWrap Groq AI Integration:
 * - AI Study Buddy Chat Assistant (askStudyBuddy)
 * - Intelligent Quiz & Flashcard Generation from PDFs/materials (generateSmartQuiz, generateSmartFlashcards)
 */
...
/**
 * Retrieve active Groq API key from localStorage or Vite environment variable.
 */
...
/**
 * AI Study Buddy Chat assistant method.
 */
export async function askStudyBuddy(question, messageHistory = []) {
  ...
  if (!apiKey || apiKey === 'paste_your_key_here' || apiKey === 'your_groq_api_key_here' || apiKey.trim() === '') {
    throw new Error('API key missing. Please configure your Groq API key in Settings or .env file.');
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        throw new Error('Invalid API Key. Please verify your Groq API key.');
...
/**
 * Generate intelligent Multiple Choice Quiz Questions from source material using Groq AI.
 */
...
/**
 * Generate conceptual Flashcards from source material using Groq AI.
 */
...
GUIDELINES:
1. "front": A clear, thought-provoking question, key concept query, or mechanism prompt.
